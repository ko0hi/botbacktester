...
def position_term(df, n=100, name="position_term", time_unit="S"):
    ...
    s = (df.co_executed_at - df.oo_executed_at).dt.total_seconds() / TIME_UNITS[
        time_unit
    ]
    s = s.rolling(n).mean()
    s.name = name
...
def execution_time(df, settle_type, n=100, name="execution_time", time_unit="S"):
    ...
    s = (
        df[f"{prefix}_executed_at"] - df[f"{prefix}_entried_at"]
    ).dt.total_seconds() / TIME_UNITS[time_unit]
    s = s.rolling(n).mean()
    s.name = name
    return s
...
def evaluation_set1(
    ...
    if subplots_kw is None:
        subplots_kw = dict(
            gridspec_kw=dict(
                height_ratios=[1] + [subpanel_size_ratio] * (panel_num - 1)
            ),
        )
    ...
    fig, axes = plt.subplots(panel_num, 1, **subplots_kw)
    ...
    ax = next(ax_iter)
    kw = ax_kw_dict.get("ls_ratio", {})
    ls_ratio(df, n=n).plot(ax=ax, **kw)
    ax.set_title("LS ratio")
    ax.set_ylim((0, 1))
